...
df = pd.DataFrame(js)
predict3 = df.iloc[64:80]
predict3['Longitude']=[120.498566,120.697299,120.741711,120.616917,
                120.646629,120.576421,120.568794,120.538839,
                120.607070,120.597876,120.641092,120.715064,
                120.677689,120.540877,120.615358,120.714881] 
predict3['Latitude']=[24.201030,24.057350,24.256586,24.162200,
                24.094264,24.269233,24.225628,24.250388,
                24.182055,24.139008,24.151958,24.139564,
                24.099611,24.150919,24.350426,24.307036]

predict3['PM25_pred'] = predict3.PM25_pred.astype(float)
predict3['PM25_pred'] = predict3['PM25_pred'].round()
predict3 = predict1.reset_index()
#predict3 未來3小時預測値
keep=predict1['Time']
Times=str(keep[0:1])
...
taichungmap_1x1 = gp.read_file("/home/hpc/3taichung1x1.geojson")         #台中1*1網格
...
fmap.save('/var/www/html/predict3')#存成 final.html
